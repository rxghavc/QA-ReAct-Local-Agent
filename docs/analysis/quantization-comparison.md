...
The project had a lower-priority theory that the default 4-bit model quantization could be degrading structured-output reliability, especially around parsing and tool-call formatting. The question was simple: if a higher-precision model was feasible within the memory budget, would it measurably improve reliability or speed enough to justify the tradeoff?
